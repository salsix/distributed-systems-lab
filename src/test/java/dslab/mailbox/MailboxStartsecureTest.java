...
public class MailboxStartsecureTest extends TestBase {
    ...
    public void sendStartsecure() throws Exception {
        ...
        try (JunitSocketClient client = new JunitSocketClient(dmapServerPort, err)) {
            ...
            //FORUM: y8pi/vUC7xzEoFsV8FXoKMB/0WfeD+k3zEBJsJ0wHwTVfJaKG7iexa565iZVMag=
                //https://tuwel.tuwien.ac.at/mod/forum/discuss.php?d=291254
            String res = client.listen();
            err.checkThat("Expected server response to be Base64 encoded", res,
                    matchesPattern("^(?:[a-zA-Z0-9+/]{4})*(?:[a-zA-Z0-9+/]{2}==|[a-zA-Z0-9+/]{3}=)?$"));
            if(!"y8pi/vUC7xzEoFsV8FXoKMB/0WfeD+k3zEBJsJ0wHwTVfJaKG7iexa565iZVMag=".equals(res)) {
                throw new RuntimeException("Wrong response: "+res);
            }

            // send encrypted "ok" (with the aes cipher) in
            //was 'g9U=' https://tuwel.tuwien.ac.at/mod/forum/discuss.php?d=291254 
            client.send("y8o=");
        }
    }
